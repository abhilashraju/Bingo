...
public:
  const char *name() const noexcept override { return "beast.http"; }
  ...
  bool equivalent(int ev,
                  error_condition const &condition) const noexcept override {
...
error_code make_error_code(error ev) {
  ...
  return error_code{static_cast<std::underlying_type<error>::type>(ev), cat};
}
} // namespace bingo
