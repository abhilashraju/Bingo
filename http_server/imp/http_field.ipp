...
struct field_table {
  using array_type = std::array<std::string_view, 353>;

  // Strings are converted to lowercase
  static std::uint32_t digest(std::string_view s) {
    std::uint32_t r = 0;
    std::size_t n = s.size();
  ...
  static bool equals(std::string_view lhs, std::string_view rhs) {
  ...
  */
  ...
  field string_to_field(std::string_view s) const {
    auto h = digest(s);
    auto j = h % N;
    int i = map_[j][0];
    std::string_view s2 = by_name_[i];
    if (i != 0 && equals(s, s2))
  ...
  const_iterator begin() const { return by_name_.begin(); }

  const_iterator end() const { return by_name_.end(); }
};

inline field_table const &get_field_table() {
  static field_table const tab;
  return tab;
}

inline std::string_view to_string(field f) {
...
std::string_view to_string(field f) { return detail::to_string(f); }

field string_to_field(std::string_view s) {
